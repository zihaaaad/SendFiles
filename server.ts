...
app.get("/api/ice-config", (_req, res) => {
  ...
  if (process.env.OFFLINE_MODE !== "true") {
    iceServers.push(
      // Two is enough for redundancy; each extra server only lengthens ICE
      // gathering, which delays exactly the local transfers that never
      // needed STUN in the first place.
